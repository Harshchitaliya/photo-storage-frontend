...
const Sidebar = (props) => {
  ...
  const renderLink = (item, index, click) => {
    return (
      <li key={index}>
        <Link
          to={item.path}
          className={`flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group ${isOpen ? "" : "justify-center"} ${pathname === item.path ? "dark:bg-gray-700"  : ""}`}
          onClick={click}
        >
          <span className="flex-shrink-0">{item.icon}</span>
          <span className={`ms-3 ${!isOpen && 'hidden'}`}>
            {item.title}
          </span>
          {!isOpen && (
            <div className="absolute left-16 scale-0 group-hover:scale-100 transition-all rounded bg-gray-800 p-2 text-xs text-white
    ...
    <>
      <Portal wrapperId="sidebar-icon">
        <button
          ...
          className={`w-5 h-5 bg-gray-200 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 z-50 ${isOpen ? "absolute top-5 left-64" : "fixed top-5 left-14"
            }`}
        >
          <span
      ...
      <aside
        className={`fixed top-0 left-0 z-40  h-screen transition-transform ${isOpen ? 'translate-x-0 w-64' : '-translate-x-full w-16'
          } sm:translate-x-0`}
      >
        <div className="h-full px-3 py-4 overflow-y-auto bg-white dark:bg-bg flex flex-col border-r border-gray-200 dark:border-gray-700">
          <div className="flex items-center ps-2.5 mb-5">
            ...
            <span className={`self-center text-xl font-semibold whitespace-nowrap dark:text-white  ${!isOpen && 'hidden'} `}>
              Your Logo
            </span>
          </div>
